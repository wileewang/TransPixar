...
class WanPipeline(BasePipeline):
    ...
    def reload_vae(self):
        ckpt_dir = self.model_config['ckpt_path']
        self.vae = WanVAE(
            vae_pth=os.path.join(ckpt_dir, self.wan_config.vae_checkpoint),
            device='cpu',
        )
        t5_model_path = self.model_config['llm_path'] if self.model_config.get('llm_path', None) else os.path.join(ckpt_dir, self.wan_config.t5_checkpoint)
        self.text_encoder = T5EncoderModel(
            text_len=self.wan_config.text_len,
            dtype=self.model_config['dtype'],
            device='cpu',
            checkpoint_path=t5_model_path,
            tokenizer_path=os.path.join(ckpt_dir, self.wan_config.t5_tokenizer),
            shard_fn=None,
        )
